...
def run_solver_in_thread(core,seed,s_name,mutant,m_name,solver,s_bin,path,timeout,opt,SEED,logic,diver_mode,debugging_info = None):
    try:
        ...
        if res == "unsat":
            ...
            if solver in ["cvc","z3"]:
                ...
                bug_copy(core,s_name,m_name,path,solver+"_soundness",SEED,seed,"Soundness Bug",bug_msg,debugging_info)
                return "unsat"
            else:
                bug_msg = "" 
                if len(opt)!=0:
            ...
            bug_copy(core,s_name,m_name,path,solver+"_invalid",SEED,seed,"Invalid Model Bug",bug_msg,debugging_info)
            res = "invalid"
            return res
        elif res == "crash":
            bug_msg = "" 
...
def run_diver_dreal(arg,benchmark,core,wait,SEED):
    ...
    while len(benchmark) != 0:
        ...
        while iter < iters:
            try:
                node = (random.choices(nodes,weights=l_score,k=1)[0]) if arg["mode"]!= "noweight" else (random.choice(nodes))
                ...
                generator = MutantGenerator(seed_info, solver_data, formula_info, file_info, mutate_info, arg["max_depth"], arg["mode"], arg["experiment"])
                ...
                if arg["debug"]:
                    ...
                    res = run_solver_in_thread(core, seed, file_name, mutant, mutant_name, arg["solver"], arg["solverbin"], mutant_path, arg["timeout"], arg["option"], SEED, arg["logic"],arg["mode"],debugging_info)
                else:
                    res = run_solver_in_thread(core, seed, file_name, mutant, mutant_name, arg["solver"], arg["solverbin"], mutant_path, arg["timeout"], arg["option"], SEED, arg["logic"],arg["mode"])
                ...
                if res == "sat":
                    ...
                    shutil.copy2(mutant, "./tmp_benchmark/core_"+str(core)+"/"+tmp_name+"_mutant_"+str(iter)+".smt2")
                    benchmark.append("./tmp_benchmark/core_"+str(core)+"/"+tmp_name+"_mutant_"+str(iter)+".smt2")
...
def run_diver(arg, benchmark, core, wait, SEED):
    ...
    while len(benchmark) != 0:
        try:
            ...
            ## (2) PREANALYSIS
            analyzer = PreAnalyzer(seed,ast, var, solver_data["model"], arg["logic"], solver_data["vars"], arg["solver_api"])
    ...
    try:
        ...
        for i in range(cores):
            ...
            if parsedArg["solver"] in ["z3","cvc"]:
                ...
                process = multiprocessing.Process(target=run_diver_dreal,args=(parsedArg,benchmark[i],core,wait,SEED))
            process.deamon = True
            SEED = str((int(SEED) + 1))
